...
Задание
Давайте определимся, зачем нужны интерфейсы?


Правильный ответ
Чтобы упростить тестирование
Да. Если все функции принимают интерфейсы в качестве параметров, тогда легко подменять настоящие объекты объектами-заглушками

Неправильный ответ
Чтобы повысить читаемость кода.
Если части кода сильно зависят друг от друга и не существует альтернативных реализаций, то проще использовать конкретные типы. Интерфейсы в таком случае только заставят ваших коллег-программистов чаще нажимать на поиск в IDE.

Правильный ответ
Чтобы поддержать разные реализации логики
Да. Например, функции авторизации, рассмотренной в уроке, можно передать как реализацию базы данных в оперативной памяти, так и реализацию базы данных, например, в PostgreSQL, MySQL.

Правильный ответ
Чтобы уменьшить связность кода.
Да. Два куска кода, которые находятся в разных модулях и не связаны между собой логически, будет проще рефакторить и изменять по отдельности.
