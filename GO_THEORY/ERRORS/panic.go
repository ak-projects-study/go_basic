...
Неправильный ответ
Паника приводит к немедленной остановке программы
Неверно, паника может быть перехвачена функцией recover.

Правильный ответ
Паника вызывается только для функции, в которой она возникла. Если паника не была перехвачена функцией recover, она будет распространяться дальше по стеку вплоть до функции main
Верно!

Тоже правильный ответ
Паника — это механизм на крайний случай: его нужно применять лишь тогда, когда других способов продолжить работу не осталось

Паника может быть вызвана только функцией panic()
Нет. Паника может возникнуть, например, при разыменовании нулевого указателя.
Задание 2
Какая конструкция верна по логике работы программы?


func A () {
    // вызываем панику
    panic()
    // останавливаем панику
    recover()
} 
Неверно, выполнение функции закончится сразу после вызова panic() — весь следующий код не будет выполнен.
...
Преобразуйте аварийную ситуацию в ошибку, которая сообщает, какая аварийная ситуация произошла: паника или деление на ноль. Проверьте работу на следующих вариантах (a,b,i):
(10, 5, 3),
(100, 7, 10),
(1, 1, 1000).
package main
