...
[1 2] 
Задание 1 из 4
Что будет, если в предыдущем примере заменить pointer receiver на value receiver — func (s IntSlice) Add(v int)?


[1 2]
Изменится только локальная копия s в методе Add.
...
Задание 2
Выберите синтаксически правильное определение метода:


func Type::String(id int) string {}
Чувствуется лёгкий привкус С++, но в Go это происходит не так.

func String(t Type) (id int) string {}
Не в том порядке.

Правильный ответ
func (t Type) String (id int) string {}
Верно!
Задание 3
Что будет при исполнении следующего кода?
package main
...
Ошибка. Программа упадёт, потому что нельзя присваивать получателю по значению константный тип
Нет. Присваивать значение можно, но присвоение будет работать только для локальной копии.
...
Неправильный ответ
5
Присвоение получателю по значению не приводит к изменению самой переменной.
Задание 4
